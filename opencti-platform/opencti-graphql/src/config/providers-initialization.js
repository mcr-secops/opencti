...
const processProviders = async () => {
    ...
    if (!config?.disabled) {
      try {
        ...
        if (strategy === StrategyType.STRATEGY_LOCAL) {
          ...
          processedProviders.push({ identifier: providerName, type: AuthType.AUTH_FORM, strategy, provider: 'local', dynamic, config: JSON.stringify(mappedConfig) });
        }
        if (strategy === StrategyType.STRATEGY_LDAP) {
          ...
          processedProviders.push({ identifier: providerName, type: AuthType.AUTH_FORM, strategy, provider: providerRef, dynamic, config: JSON.stringify(mappedConfig) });
        }
        // SSO Strategies
        if (strategy === StrategyType.STRATEGY_SAML) {
          ...
          processedProviders.push({ identifier: providerName, type: AuthType.AUTH_SSO, strategy, provider: providerRef, dynamic, config: JSON.stringify(mappedConfig) });
        }
  ...
  if (!hasLocal) {
    ...
    processedProviders.push({ identifier: INTERNAL_SECURITY_PROVIDER, type: AuthType.AUTH_FORM, strategy: StrategyType.STRATEGY_LOCAL, provider: 'local', dynamic: false, config: '' });
  }
